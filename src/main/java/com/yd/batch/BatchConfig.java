...
import com.yd.config.CalendarConfig;
...
public class BatchConfig {
	...
	public List<Calendar> defineCalendars(SchedulerFactoryBean schedulerFactory) {
		if (calendarConfig == null
				|| calendarConfig.getCalendarNames() == null) {
			return Collections.emptyList();
		}
		List<Calendar> calendars = new LinkedList<>();
		for (String name : calendarConfig.getCalendarNames()) {
			HolidayCalendar cal = new HolidayCalendar();
			try {
				for (Date date : calendarConfig.getDatesForCalendar(name)) {
